...
selected_feature_columns = train_valid_df.columns[
    train_valid_df.columns.get_loc(0) : train_valid_df.columns.get_loc("Class")
].tolist()
...
confusion_matrix_df.to_excel(f"confusion_matrix.xlsx")
...
if verbose >= 1:
    print(
        f"train accuracy: {train_acc:.4f}, validation accuracy: {mean_validate_score:.4f}, test accuracy: {test_acc:.4f}"
    )
...
best_params_df.to_excel(f"best_params.xlsx", index=False)
